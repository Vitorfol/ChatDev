...
class ChatAgent(BaseAgent):
    ...
    def step(
        ...
        if num_tokens < self.model_token_limit:
            ...
            # Para modelos Ollama e OpenAI, sempre tratar como dict
            if not isinstance(response, dict):
                raise RuntimeError("Model backend returned unexpected struct")
            output_messages = [
            ...
            info = self.get_info(
                response.get("id", "ollama-response"),
                response.get("usage", {}),
                [str(choice.get("finish_reason", "")) for choice in response.get("choices", [])],
                num_tokens,
            )
